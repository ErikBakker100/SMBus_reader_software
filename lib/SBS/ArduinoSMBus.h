...
/**
 * @struct ManufacturerPermFailure
 * @brief A struct used in the ManufacturerBatStatus struct to hold permanent failure cause.
 * To be filled when Permanent Failure state is true in the ManufacturerBatStatus struct
 */
struct ManufacturerPermFailure {
  uint16_t raw;                 /**< Data Read. */
  bool chg_fet;                 /**< On if 1 (True), Off if 0 (False). */
  bool dsg_fet;                 /**< On if 1 (True), Off if 0 (False). */
  bool fuse;                    /**< Fuse is blown if enabled via DF:Configuration:Register(64):Permanent Fail Cfg(6). */
  bool cell;                    /**< Cell imbalance failure. */
  bool satety;                  /**< Safety voltage failure. */
  bool fet;                     /**< FET failure. */
};
 ...
 */
struct ManufacturerBatStatus {
  struct ManufacturerPermFailure failure {0};
  bool wakeup;
  bool normaldischarge;
  bool precharge;
  bool charge;
  bool chargetermination;
  bool faultchargeterminate;
...
public:
  ...
  uint16_t manufacturerAccessFirmware(uint16_t); // command 0x00
  uint16_t manufacturerAccessHardware(uint16_t); // command 0x00
  ManufacturerBatStatus manufacturerAccessBatStatus(uint16_t); // command 0x00
  void manufacturerAccessSealDevice(uint16_t); // command 0x00
